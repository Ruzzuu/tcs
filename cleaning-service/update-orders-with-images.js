...
// 1. Add CloudinaryImage import after PhoneAutocomplete import
content = content.replace(
  `import PhoneAutocomplete from '@/components/PhoneAutocomplete';`,
  `import PhoneAutocomplete from '@/components/PhoneAutocomplete';\nimport { CloudinaryImage } from '@/types';`
);

// 2. Add image state variables after existing state
const itemsStatePattern = /(\s+const \[items, setItems\] = useState<FormItem\[\]>\(\[\n\s+\{ service: '', quantity: 1, notes: '' \}\n\s+\]\);)/;
content = content.replace(itemsStatePattern, `$1
...
// 3. Add handleImageUpload and handleDeletePhoto functions after decrementQuantity
const decrementPattern = /(\s+const decrementQuantity = \(index: number\) => \{\n\s+updateItem\(index, 'quantity', items\[index\]\.quantity - 1\);\n\s+\};)/;
const imageHandlers = `
...
content = content.replace(decrementPattern, `$1${imageHandlers}`);

// 4. Add proofOfWork to payload in handleSubmit
const payloadPattern = /(\s+items: validItems\.map\(item => \(\{\n\s+itemType: item\.service,\n\s+customItemType: '',\n\s+quantity: item\.quantity,\n\s+notes: item\.notes\n\s+\}\)\))/;
content = content.replace(payloadPattern, `$1,
        proofOfWork: {
...
// 5. Add image upload UI before submit button
const submitButtonPattern = /(\s+\{\/\* Submit Button \*\/\})/;
const imageUploadUI = `
...
console.log('✅ Successfully updated admin orders page with image upload functionality');
